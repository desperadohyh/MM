function robot=robotproperty_MMD_s(id, z0_, Ts)
        ...
        robot.nlink = nlink;
        robot.umax=10;
        robot.thetamax=[-pi/2,pi/2; -pi/2,(pi/2)*0.75; -pi*0.75,pi*0.75];
        robot.thetadotmax=[1;1;1;1];
        robot.margin=0.05;
        robot.delta_t=Ts;
        robot.tua_max = 3;
        
        %The length of each links and DH parameter and base
        robot.l=[0.04; 0.12228; 0.1245; 0.08];
        robot.lc=[0.02; 0.0618; 0.0645; 0.05 ];
        theta = [ 0 ; 0  ; 0 ; 0];
        robot.theta = theta;
        robot.T = [0   0  0    0.03  0.13;
                   0   0  0   -0.12    0;
                   0   0  0.04   0     0]; %% distance between coordinates
        T = robot.T;
        robot.Tc = [  0   0.015   0.07  0.05;
                      0   -0.07    0      0;
                      0.4   0      0      0]; % center of mass of each link w.r.t. local coordinates
        robot.DH=[0, 0.034, 0.0825, 0 ;
                  0, 0.04,  0 , 3*pi/2;                    
                  0, 0.12, 0.0235, 0;
...
%                   0, 0.12, 0.0235, 0;
%                   0, 0, 0.1245, 0];%theta,d,a,alpha
        robot.m = [ 0.09 ; 0.13 ; 0.12 ;0.4]; % mass of each link
        ...
        robot.cap={}; 
        robot.cap{1}.p=[ 0 0;0 0;0 0.04];
        robot.cap{1}.r=0.025;
        
        robot.cap{2}.p=[0.011 0.011;0 -0.12;0 0];
        robot.cap{2}.r=0.025;
        
        robot.cap{3}.p=[0 0.124;0 0;0 0];
        robot.cap{3}.r=0.025;        
        
        robot.cap{4}.p=[0 0.12;0 0;0 0];
        robot.cap{4}.r=0.025;
